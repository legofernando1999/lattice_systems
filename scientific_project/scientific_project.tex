...
\begin{align*}
    \left( H - \lambda \right) v_1 &= \left( H - \lambda \right) \mathbf{1}_{B_r(x)} v_1 && &\text{(since $v_1$ is supported on $B_r(x)$)} \\
    &= \mathbf{1}_{B_{r + m}(x)} \left( H - \lambda \right) \mathbf{1}_{B_r(x)} v_1 && &\text{(by the finite range of $H$)} \\
    &= Q_{r, \lambda, x} v_1 \\
    &= s_1 u_1
\end{align*}
...
For the proof of this theorem see \cite{paulhege2025}.
...
For a derivation of either of these algorithms, see \cite{paulhege2025}.
...
\begin{equation*}
    H_{\text{free}} \psi(x) = -\frac{1}{2}\frac{\diff}{\diffx}\psi(x) = E \psi(x), \quad \text{for} \; 0 \leq x \leq L, \quad \text{with} \; \psi(0) = \psi(L)
\end{equation*}

requires that we select an integer $N > 0$ and divide the interval $[0, L]$ into $(N - 1)$ equal subintervals whose endpoints are the mesh points $x_i = i \Delta x$, for $i = 0, 1, 2, \dots, N - 1$, where $\Delta x = L / (N - 1)$. Using a centered-difference formula for $\psi'(x_i)$ together with the boundary conditions, we get
