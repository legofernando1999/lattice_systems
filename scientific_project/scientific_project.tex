...
In this chapter, we shall present the general framework developed in \cite{paulhege2025} for approximating the (pseudo)spectra of linear operators with full error control, and explain how each mathematical object is employed in our specific case.
...
for $\Delta x = 1$. We can visualize its spectrum through a kernel density estimation (KDE) plot\footnotemark, as shown in figure \ref{fig:free_hamiltonian}.
...
Now, in order to validate the pseudospectral inclusion bound for this operator, first we need to take an uneven section. In fact, given the simplicity of $H_N$, we can center it at any point. For instance, let $r \in \{ 50, 100, 150, 200, 250 \}$, $\lambda \in \mathbb{R}$, and $x = 500$, then an uneven section looks like so
...
\begin{center}
    \begin{tabular}{ |c||c|c|c| }
        \hline
        \multicolumn{4}{|c|}{Bounds on $\rho_H(\lambda)$} \\
        \hline
        $\left( r, \lambda \right)$ & $d\left(\lambda, \sigma(H)\right)$ & Upper bound & Lower bound \\
        \hline
