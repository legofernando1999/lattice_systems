...
As shown in \cite{paulhege2025}, as long as we are dealing with operators with finite local complexity, it is always possible to compute the spectrum in the normal case, and the pseudospectrum in the nonnormal one. In this section we provide the theorems that led to these results.

\subsubsection{The Pseudospectral Inclusion Bound (PIB)}

An upper bound on $\rho_H$ can be obtained by performing a singular value decomposition on the rectangular matrix $\left( H_{yz} \right)_{y \in B_{r + m}(x), \, z \in B_{r}(x)}$, where $x \in \mathbb{R}^n$ and $r, \, m > 0$ are fixed numbers.

\textbf{Remark.} Recall that a \textit{singular value decomposition} (SVD) of a matrix $A \in \mathbb{C}^{m \times n}$ is a factorization into a product of matrices $A = U S V^*$, where $U \in \mathbb{C}^{m \times m}$ and $V \in \mathbb{C}^{n \times n}$ are unitary, and $S \in \mathbb{C}^{m \times n}$ is diagonal. The matrix $S$ contains the square roots of the nonnegative eigenvalues of both $AA^*$ and $A^*A$, and these are the \textit{singular values} of $A$ \cite{gilbertstrang}.
...
Now, in order to validate the pseudospectral inclusion bound for this operator, first we need to take an uneven section. In fact, given the simplicity of $H_{\text{free}}$, we can center it at any point. For instance, let $r \in \{ 50, 100, 150 \}$, $\lambda \in \mathbb{R}$, and $x = 500$, then an uneven section looks like so
...
\begin{center}
    \begin{tabular}{ |c||c|c| }
        \hline
        \multicolumn{3}{|c|}{Pseudospectral Inclusion Bound (PIB)} \\
        \hline
        $\left( r, \lambda \right)$ & $d\left(\lambda, \sigma(H)\right)$ & $\varepsilon_{r, \lambda, 500}$ \\
        \hline
        $(50, -0.1)$ & 0.100000 & 0.100507 \\
        $(100, -0.1)$ & 0.100000 & 0.100125 \\
        $(150, -0.1)$ & 0.100000 & 0.100055 \\
        \hline
        $(50, 0.5)$ & 0.000906 & 0.026657 \\
        $(100, 0.5)$ & 0.000906 & 0.013402 \\
        $(150, 0.5)$ & 0.000906 & 0.008979 \\
        \hline
        $(50, 1.2)$ & 0.002025 & 0.029876 \\
        $(100, 1.2)$ & 0.002025 & 0.015225 \\
        $(150, 1.2)$ & 0.002025 & 0.010161 \\
        \hline
        $(50, 2.3)$ & 0.300005 & 0.300494 \\
        $(100, 2.3)$ & 0.300005 & 0.300123 \\
        $(150, 2.3)$ & 0.300005 & 0.300055 \\
        \hline
    \end{tabular}
\end{center}
\endgroup
...
\vspace{0.5cm}
These data clearly show that the PIB is valid in the present case. Moreover, from them we can see that increasing the window size ($r$) of the uneven section lowers the bound. The reason for this is that a larger window will contain more of the support of the eigenvector of $H_{\text{free}}$ corresponding to the eigenvalue closest to our chosen $\lambda$, and so, the SVD can produce a better approximation to both the eigenvector and the eigenvalue. Another interesting conclusion we can draw from them is that choosing a $\lambda$ outside of the range of the spectrum yields a smaller bound because the eigenvectors with eigenvalues at the boundary are more localized.

\newpage

\section{Conclusion and Future Work}

A lower bound on $\rho_H$ and a general algorithm for approximating the pseudospectra of infinite-volume operators with FLC were proposed in \cite{paulhege2025}, and here we tested the pseudospectral inclusion bound against the free Hamiltonian with positive results.

Future work should focus on validating the PIB and the spectral gap bound against different Hamiltonians, e.g., random Hamiltonians, Hamiltonians emerging from quasicrystal systems, etc. Additionally, combining the computational tools developed for this work with the bounds on the lower norm function should enable us to prove spectral gaps of infinite-volume one-body operators.
